  ...
  try {
    ...
    try {
      ...
      if (err && err.code === "auth/account-exists-with-different-credential") {
        ...
        setError("invSignInError", friendlyAuthError(err) + debugSuffix(err));
      }
    }
...
// TEMPORARY DEBUG AID (2026-09-06 investigation): auth/internal-error is
// a catch-all -- the friendly message above can't say more than "check
// the console" because the SDK doesn't expose *which* underlying problem
// it is through err.code alone. Every other angle (API key restrictions,
// OAuth client origins/redirect URIs, consent screen project settings)
// has been checked and is correctly configured, so the next step is
// seeing whatever extra detail Firebase attaches to the error object
// itself, directly on screen -- there's no practical way to get a
// phone's browser devtools console otherwise. Safe to remove this once
// the real cause is found and fixed; it deliberately is not pretty.
function debugSuffix(err) {
  ...
  try {
    if (err.customData) {
      const cd = Object.assign({}, err.customData);
      delete cd.appName;
      const cdStr = JSON.stringify(cd);
      if (cdStr && cdStr !== "{}") parts.push("customData=" + cdStr);
    }
  } catch (e) { /* ignore -- debug aid only, must never itself throw */ }
  return parts.length ? (" [DEBUG: " + parts.join(" | ") + "]") : " [DEBUG: no extra detail on the error object]";
}
  ...
  try {
    ...
    if (err && POPUP_UNSUPPORTED_CODES.has(err.code)) {
      try {
        ...
        setError(errorId, friendlyAuthError(redirectErr) + debugSuffix(redirectErr));
      }
      return;
    }
    setError(errorId, friendlyAuthError(err) + debugSuffix(err));
  }
}
